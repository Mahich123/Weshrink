...
export default function CreateUrlForm() {
  const form = useForm<z.infer<typeof createUrlFormSchema>>({
  ...
  const submitUrl = async (values: z.infer<typeof createUrlFormSchema>) => {
    console.log(values)
  }
  ...
  return (
    <Card className="w-full max-w-[20rem] md:max-w-md mx-auto bg-[#605678]/40 border-[#D9D9D9]">
      ...
      <CardContent className="text-white">
        <Form {...form}>
          <form onSubmit={form.handleSubmit(submitUrl)} className="space-y-4">
            <FormField
